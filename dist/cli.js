...
async function paginateAll(baseRequest, apiFunction, resultExtractor, initialAccumulator, resultAccumulator, options = {}) {
  ...
  if (logResults) {
    console.log(`\uD83D\uDCCA Total ${requestName.toLowerCase()} fetched: ${totalResults} (${pageCount} pages)`);
  }
  return accumulated;
}
...
program2.name("redpanda-migration").description("Migrate roles and principals from Redpanda Cloud to Core").version("1.0.0");
